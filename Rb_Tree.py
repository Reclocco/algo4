...
class Rb_Tree(Structure):
    ...
    def find(self, k):
        node = self.__search_tree_helper(self.root, k)
    ...
    def successor(self, x):
        node = self.successor_node(x)
        if node is not None:
            print(node.data)
        else:
            print()

    def successor_node(self, value):
        x = self.find_node(value)

        if x.right != self.TNULL:
            return self.min_rel(x.right)
